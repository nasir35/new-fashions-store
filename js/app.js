...
// loadProducts function will get all the data and will pass to showProducts function to display in webpage
const loadProducts = () => {
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    // here was a bug of mis-spelling for which image was not loading
...
let count = 0;
// addToCart function created to count product and to calculate cost based on added product in cart 
...
// getInputValue function capture an element with id and return it's innerText converting to a precised floating numbers
const getInputValue = (id) => {
  const element = document.getElementById(id).innerText;
  // below, i am using Math.abs function to convert string (returned by .toFixed function) to positive number and using .toFixed function to maintain 2 decimal precision
  const converted = Math.abs(parseFloat(element).toFixed(2));
  return converted;
};
...
const updatePrice = (id, value) => {
  ...
  const convertPrice = Math.abs((value).toFixed(2)); /*explained in earlier function*/
  const total = convertedOldPrice + convertPrice;
  document.getElementById(id).innerText = Math.abs(total.toFixed(2)); /*explained in earlier function*/
};

// set innerText function
const setInnerText = (id, value) => {
  document.getElementById(id).innerText = Math.abs(value.toFixed(2)); /*explained in earlier function*/
};
...
const updateTotal = () => {
  ...
  document.getElementById("total").innerText = Math.abs(grandTotal.toFixed(2)); /*explained in earlier function*/
};
loadProducts();